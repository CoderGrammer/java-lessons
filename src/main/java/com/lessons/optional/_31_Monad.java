...
/*
 - Is it a monad?
     - Example Functor with a flatMap method...
     ...
     - Three laws need to be satisfied:
        1. Left identity
            - If we take a value, put it in a default context with return and then feed
             it
              to a function by using >>= (flatMap), it's the same as just taking the
              value and applying the function to it.
            - 'return x >>= f' same as 'f x'
        2. Right identity
            - If we have a monadic value and we use >>= (flatMap) to feed it to return,
     ...
     - Where things become problematic:
         ...
         - Flat map is more conducive to changing the shape of the box e.g. if you had
